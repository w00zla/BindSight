...
11. **Keyboard + gamepad** (2026-09-10): every feature now covers SC's one
    ...
    `gamepad_seen`. Each device tile has an eye button hiding its stage tile
    (`bindsight.stage.hidden`, hardware ids). **The SC-data cache shape
    changed** (`Action` gained two fields): the old cache fails to
    deserialise with a WARN and is re-extracted once.
...
- **Keyboard + gamepad (2026-09-10), partly GUI-verified**: the app starts,
  ...
  rough edges: in Devices mode with the keyboard map open, Escape / Enter /
  Delete / Backspace also trigger the editor shortcuts; in Monitor mode the
  capture `preventDefault`s webview shortcuts; both keyboard maps match, the
  first by name (DE) wins until the user picks; the keyboard cannot be
